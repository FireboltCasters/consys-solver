...
const url = config.package.repositoryURL.replace('.git', '');

packageJSON.bugs.url = url + '/issues';
packageJSON.homepage = url + '#readme';

const tokens = url.split('/');
const organization = tokens[tokens.length - 2];
...
sonarConfig +=
  'sonar.coverage.exclusions=src/tests/**/*,src/ignoreCoverage/**/*,babel.config.js' +
  '\n';
sonarConfig +=
  'sonar.exclusions=src/tests/**/*,src/ignoreCoverage/**/*,babel.config.js' +
  '\n';
...
fs.writeFile('./sonar-project.properties', sonarConfig, err => {
  if (err) throw err;
